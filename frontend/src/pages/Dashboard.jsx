    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">
          Buen día, {user?.name ?? 'Administrador'} 👷
        </h2>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard icon={UsersIcon} label="Personal en obra" value="14" hint="3 maestros · 11 obreros" tone="emerald" />
        <StatCard icon={CurrencyDollarIcon} label="Costo consolidado" value={fmtCOP(462_800_000)} hint="Materiales + servicios del periodo" tone="sky" />
      </div>
          ...
          {incidencias.map((i) => (
            <div key={i.id} className="rounded-xl border border-slate-200 p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="badge bg-slate-100 text-slate-600">{i.estado.replace('_', ' ')}</span>
              </div>
              ...
              <p className="mt-2 text-[11px] text-slate-400">{i.fecha}</p>
            </div>
          ))}
        </div>
      </div>
